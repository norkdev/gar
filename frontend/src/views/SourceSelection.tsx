...
export function SourceSelection({
  ...
  const candidates: Candidate[] = state.pending_payload.candidates ?? []
  ...
  return (
    <main>
      ...
      <p className="muted">
        ...
        the report will say so honestly.
      </p>

      {sorted.length === 0 && (
        <p className="muted" style={{ fontStyle: 'italic' }}>
          No candidates were returned. (This can happen if the agent decided
          it had enough context, or if upstream sources rate-limited.)
        </p>
      )}
      ...
      {sorted.map((c) => {
        ...
        return (
          <div
            key={id}
            className={isAdopted ? 'candidate adopted' : 'candidate'}
          >
      ...
      <p style={{ marginTop: '1rem' }}>
        <button onClick={submit} disabled={busy}>
          {busy
            ? 'Composing report…'
            : `Adopt ${adoptedCount} and compose report`}
        </button>
      </p>
