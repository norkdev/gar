...
- `infra/` — AWS CDK (Python). `DataStack` (DynamoDB `gar-runs` + state S3 bucket) and `BackendStack` (Lambda arm64 + Function URL `NONE` + CORS, async self-invoke IAM, two Secrets Manager secrets) are **deployed** in `ap-northeast-1` (account `733287383921`). `Workflow` / `Frontend` / `Auth` stacks still scaffolds. Deploy creds: `deploy` profile (short-lived; `eval "$(aws configure export-credentials --profile deploy --format env)"` before `cdk`). **Deploy / re-deploy / destroy runbook: `docs/deploy.md`.**
