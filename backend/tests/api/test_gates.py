...
def test_approve_concept_advances_to_source_selection(
    ...
    response = api_setup["client"].post(
        f"/runs/{run_id}/gates/concept", json={}
    )
...
def test_approve_concept_on_unknown_run_returns_404(
    api_setup: dict[str, Any]
) -> None:
    response = api_setup["client"].post(
        "/runs/missing/gates/concept", json={}
    )
    assert response.status_code == 404


def test_approve_concept_on_wrong_status_returns_409(
    api_setup: dict[str, Any]
) -> None:
    ...
    second = api_setup["client"].post(
        f"/runs/{run_id}/gates/concept", json={}
    )
    assert second.status_code == 409


def test_select_sources_advances_and_records_adopted(
    api_setup: dict[str, Any]
) -> None:
    run_id = _start_run(api_setup)
...
def test_select_sources_on_wrong_status_returns_409(
    api_setup: dict[str, Any]
) -> None:
    run_id = _start_run(api_setup)
    response = api_setup["client"].post(
...
def test_approve_report_completes_and_saves_to_disk(
    api_setup: dict[str, Any]
) -> None:
    run_id = _start_run(api_setup)
    api_setup["llm"].responses.extend([
...
def test_approve_report_on_wrong_status_returns_409(
    api_setup: dict[str, Any]
) -> None:
    run_id = _start_run(api_setup)
    response = api_setup["client"].post(f"/runs/{run_id}/gates/report")
