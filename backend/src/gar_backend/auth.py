...
v1 left this as a stub. v2.0 enforces a shared API key so the Function URL can
drop IAM signing (auth_type NONE) and still not be open to the world — the MCP
server and browser send the key in a header instead of SigV4-signing.
...
Identity (per-user auth, Cognito) is a later phase; this is a single shared
app key, not a user credential.
"""
